...
def geometry_penalty(coords: np.ndarray,
    ...
    if len(t_le) > 0:
        ...
        if t_le_min < min_thickness_le:
            return 1000.0, {
                "reason":  "too thin (LE zone)",
                "t_min":   t_le_min,
                "zone":    "LE [0.05,0.15]",
                "floor":   min_thickness_le,
            }
    else:
        t_le_min = float("nan")
    ...
    if len(t_mid) > 0:
        ...
        if t_mid_min < min_thickness_mid:
            return 1000.0, {
                "reason":  "too thin (Mid zone)",
                "t_min":   t_mid_min,
                "zone":    "Mid (0.15,0.75]",
                "floor":   min_thickness_mid,
            }
    else:
        t_mid_min = float("nan")

    # TE zone: x ∈ (0.75, 0.95]  <-- extended from old 0.90
    te_zone_mask = (xg > 0.75) & (xg <= 0.95)
    t_te         = thickness[te_zone_mask]
    if len(t_te) > 0:
        t_te_min = float(np.min(t_te))
        if t_te_min < min_thickness_te:
            return 1000.0, {
                "reason":  "too thin (TE zone)",
                "t_min":   t_te_min,
                "zone":    "TE (0.75,0.95]",
                "floor":   min_thickness_te,
            }
    else:
        t_te_min = float("nan")
